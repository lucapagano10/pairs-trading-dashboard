...
class Correlation():
    ...
    def get_log_correlation(self, cleared_df:pd.DataFrame, min_correlation: float, save: bool = True) -> pd.DataFrame:
        ...
        corr_df.to_csv(os.path.join(path,'..','./data/processing/corr_output.csv'), index=False) if save else None
        return corr_df
